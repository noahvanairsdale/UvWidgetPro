...
def calculate_uv_index(latitude, longitude, current_time, cloud_cover=0, ozone_column=300, aod=0.1):
    """
    ...
    try:
        ...
        return round(avg_uv * (1 - 0.7 * cloud_cover / 100), 1)
    
    # Parameters adjusted for mid-latitudes
    uv_max = 12.0  # Slightly reduced based on typical Livonia values
    ...
    cloud_adjustment = 1 - 0.7 * (cloud_cover / 100)  # Reduce UV by up to 70%
    ...
    if solar_zenith_angle < 75:
        uv_index = uv_max * (math.cos(math.radians(solar_zenith_angle)) ** alpha) * seasonal_adjustment * cloud_adjustment * time_adjustment * altitude_adjustment * ozone_adjustment * aerosol_adjustment
    else:
        uv_index = uv_max * 0.2 * ((90 - solar_zenith_angle) / 15) * seasonal_adjustment * cloud_adjustment * time_adjustment * altitude_adjustment * ozone_adjustment * aerosol_adjustment
